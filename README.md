...
- `src/spyInputs.py`: Manages the creation of "interesting" inputs for input checking functions.
...
- Featuring:
  - Pink line highlight when `#spy` python comments are found, indicating that **spy** will run on the tagged function. Hovering these tags will result in a tooltip popup with Radon code statistics if enabled.
  - Unobtrusive green line highlight (TBD: coming soon) to indicate code coverage on `#spy`-tagged functions.
  - Unobtrusive red line highlight (TBD: coming soon) to indicate lack of code coverage on `#spy`-tagged functions.
#### 4. Analysis
- TBD
#### 5. Testing
- You do not need to write coverage or boundary condition tests! The biggest advantage that **spy** provides is that it will automatically (attempt to) test your code for failure points. Testing is automatically performed when you save a file that includes a function tagged with `#spy`.
